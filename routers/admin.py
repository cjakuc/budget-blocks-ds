...
def get_current_username(
        credentials: HTTPBasicCredentials = Depends(security), tags=["admin"]):
...
# End point for main admin interface
...
# Endpoint for reset master confirmation page
...
# Endpoint for reset user confirmation page
...
# Endpoint to reset the master table
@router.get("/admin/reset_master/", tags=["admin"])
def reset_master(request: Request,
                 username: str = Depends(get_current_username)):
    resetMaster()
...
async def testing(request: Request,
    ...
    if (Plaid_cat != 'None') & (Destination != 'None'):
        ...
        # Get the 5 most recent changes in the changelog
        changes = masterChanges()

        # Redirect to edit_success page which easily allows the admin to return
        # to the main admin panel or go directly to the admin/db page
    ...
    # one for displaying readable Plaid categories to the user and one for
    # using to change the DB later
    html_dict, spaces_dict = DictHTML(Dict)

    # If a category is chosen, save a zipped version of the two different
    # lists (readable and useful) of values to 'value_list'
    if Cat != 'None':
    ...
    # Return the admin.html template with the appropriate variables in the
    # context parameter
...
# Route that allows us to check the current values of the Postgres master table
